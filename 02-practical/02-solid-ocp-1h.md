...
→ 要は **「継承で機能を追加していきましょう、ポリモーフィズムしましょう」**
...
変わりやすい仕様を削ぎ落とした本質(=高抽象度)と、それ以外とで分離する

* 要求の追加でコードが増えた
* 仕様の変更でコードが変わった
* もっと良い書き方を思いついた etc.

**…でもどうやって安定・不安定の境界を見出す？**

* 安定度・抽象度等価の原則(SAP)で考える。コード変更が起きにくい=安定度が高い=抽象度が高い場所

>>> SAP: Stable Abstractions Principle. パッケージの安定度と抽象度には相関関係がある

<!-- OCP: 将来起きるであろう変動に備える原則。 -->

---

## 演習

それぞれ構造を表した図から、OCPを適用する境界に線を引いてみよう
...
* 切り分けが少なすぎる: 神クラス/モジュールの発生
* 多すぎる: 不要なコード増、メンテナンス性低下

![bg right 60%](assets/02-ocp-goldilocks.png)

<!-- 3びきのくま（英語: the Three Bears）またはゴルディロックスと3匹のくま（Goldilocks and the Three Bears）は、イギリスの有名な童話
３匹のクマが暮らす家で、留守中に女の子が迷い込む。女の子はおかゆ、椅子、ベッドそれぞれ３つある中でちょうどいいものを選んでいくという話。
ゴルディロックスゾーンは「ハビタブルゾーン」とも言われ、宇宙での生命の生存可能な領域のことを指す
 -->
